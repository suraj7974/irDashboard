...
export default function PDFViewer({ fileUrl, onClose, title }: PDFViewerProps) {
  ...
  const defaultLayoutPluginInstance = defaultLayoutPlugin({
    ...
    renderToolbar: (Toolbar) => (
      <Toolbar>
        {(slots) => {
          const { ZoomIn, ZoomOut, Search, GoToNextPage, GoToPreviousPage, CurrentPageInput, NumberOfPages } = slots;
          return (
            <>
                ...
                <Search>
                  {(renderSearchProps) => (
                    <>
                      ...
                      <button onClick={renderSearchProps.jumpToPreviousMatch} disabled={renderSearchProps.currentMatch <= 0}>
                        Prev
                      </button>
                      <button onClick={renderSearchProps.jumpToNextMatch} disabled={false}>
                        Next
                      </button>
                    </>
                  )}
    ...
    <div className="fixed inset-0 z-50 bg-black bg-opacity-75 flex items-center justify-center">
      <div className="bg-white rounded-lg shadow-xl max-w-6xl max-h-[90vh] w-full mx-4 flex flex-col">
        ...
        <div className="flex justify-between items-center p-4 border-b">
          <h3 className="text-lg font-semibold text-gray-900 truncate">{title || "PDF Viewer"}</h3>
